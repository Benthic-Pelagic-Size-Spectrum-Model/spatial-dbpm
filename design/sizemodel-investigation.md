...
|---|---|---|---|
| `dbpm_isimip_3b` | CMIP5 `sizemodel()` (R) | `master` | #2 |
...
`scripts/useful_functions.py` (`new_features`), an RK4 integrator with
`rk4_substeps=4` (= the "weekly = each monthly step ×4" the team describes). It
carries the identical bug in the same three integral types — feeding convolutions
(`feeding_satiation_rates`), predation-mortality convolutions (`mortality_calc`),
and the reproduction egg-sum (`tot_biomass_calc`) — with the advection
`1/np.log(10)` Jacobian correctly present. PR #16 restores `np.log(10)` on those
six integrals (compiles; not runtime-validated — no gridded zarr forcing on hand).
The Python also has `depth_corr`/`benthic_habitat_depth` output multipliers (the
areal-units offset of §6a's reconciliation) and an optional `force_finite` NaN
mask (the team's runs set it `False`). The **R** `gridded_sizemodel()` on `main`
(`dbpm_model_functions.R`, PR #15) is the legacy counterpart; the `new_features`
R `useful_functions.R` has only aspatial `sizemodel()` (PR #14) plus the gridded
param/forcing plumbing.